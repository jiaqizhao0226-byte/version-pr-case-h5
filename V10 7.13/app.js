...
function renderTagDamageChart(){
  ...
  wrap.innerHTML = `<div class="triHeatAxis"><span>核心矛盾</span><span>伤害档位分布（按案例数堆叠）</span></div>
    ...
    <div class="chartInsights"><b>洞察：</b>
      <p style="margin:0 0 6px;font-size:12px;">（各矛盾类型的有效样本约 4–9 个，占比为方向性参考。）</p>
      <ul>
        ...
        <li data-tag="情感/价值观争议"><b>情感/价值观争议</b>呈两极分化（有效 9 例：高 4 / 中 1 / 低 4，高伤占比 44%）——触及情感契约底线则伤及留存，未触及则只是情绪宣泄。</li>
...
function renderPrDamageChart(){
  ...
  wrap.innerHTML = `<div class="gdRows">${html}</div>${legend}
    <div class="chartInsights"><b>洞察：</b>
      ...
      <ul>
        <li data-tag="立刻滑跪,前期冷处理+后期滑跪"><b>滑跪（含立刻滑跪和后期滑跪）</b>的伤害分布比其它处置更偏向中 / 高档（合并后 14 例中，中 / 高伤害 10 例、低伤害仅 1 例、无数据 3 例），且触及核心付费 / 契约的案例几乎都会滑跪。这说明滑跪更多是<b>事件已被判定为"动了根基"后的止损选择</b>——不是"滑跪导致高伤害"，而是"判断伤到核心才滑跪"（选择效应，非因果）。需注意：单看"立刻滑跪"一项，伤害分布其实较分散（高/中/低都有），滑跪速度本身是姿态选择，不能直接当作严重度信号。</li>
        <li data-tag="冷处理"><b>冷处理</b>以低伤害为主（7 例中 5 例低伤害、1 例无数据），说明它多用于官方判断"没动到核心盘"的场景。风险在于误判：剩下 1 例正是<b>世界之外</b>——把冷处理用在核心契约被击穿的事上，成了全库唯一的冷处理高伤，错过最佳止损窗口。</li>
...
function renderC2TagDamage(){
  ...
  all.forEach(c => caseTags(c).forEach(t=>{
    (agg[t] = agg[t] || {high:0,mid:0,low:0,na:0});
    agg[t][dmgTier3(c.damage)]++;
  }));
  ...
  rows.forEach((r,i)=>{
    ...
    [['low',C.low],['mid',C.mid],['high',C.high]].forEach(([k,col])=>{
      ...
      yTop-=h;
      bars+=`<rect x="${bx.toFixed(1)}" y="${yTop.toFixed(1)}" width="${bw}" height="${h.toFixed(1)}" fill="${col}"><title>${esc(r.t)} · ${k==='high'?'高':k==='mid'?'中':'低'}伤害 ${v}</title></rect>`;
  ...
  rows.forEach((r,i)=>{
    ...
    dots+=`<circle cx="${cx.toFixed(1)}" cy="${py.toFixed(1)}" r="5" fill="#fff" stroke="${LINE}" stroke-width="3"><title>${esc(r.t)} 中高伤害率 ${pct}</title></circle>`;
  ...
  const svg=`<svg viewBox="0 0 ${W} ${H}" width="100%" style="max-width:660px;display:block;margin:0 auto">${grid}<line x1="${padL}" y1="${padT+plotH}" x2="${W-padR}" y2="${padT+plotH}" stroke="var(--line)"/>${bars}${line}${dots}${xlabels}${rlabels}</svg>`;
...
// —— C2 图2b · 上线期高伤 8 案根因分类（色条 + 可点击案例 chip）——
const C2_LAUNCH_ROOTS = [
  { name:'付费诚信问题', color:'#B85450', cases:[['shijiezhiwai-money','世界之外'],['fengzhigu-refund','枫之谷']], desc:'涨价 / 暗改数值，单方面削弱已付费价值' },
  { name:'产品定位错位', color:'#D99A3E', cases:[['hpmagic-pay','哈利波特'],['gf2-daiyan','少女前线2'],['shediao-bjd','射雕']], desc:'定价 / 角色 / 建模定位与核心用户预期冲突' },
  { name:'内容 / 资格合规触线', color:'#8E7CC3', cases:[['chenbai-forbidden','尘白禁区'],['helldivers2-psn','绝地潜兵2']], desc:'擦边触监管 / 强绑 PSN 侵犯已购权益' },
...
function renderC2LaunchRoots(){
  ...
  wrap.innerHTML = C2_LAUNCH_ROOTS.map(r=>{
    ...
    return `<div class="c2RootRow">
      ...
      <div class="c2RootBar"><div class="b" style="width:${barW}px;background:${r.color}"></div><span class="cnt" style="color:${r.color}">${r.cases.length}</span></div>
...
const C2_REDLINE = [
  {g:'情怀 IP', color:'#D99A3E', contract:'"记忆"', line:'定位偏离即信任崩塌',
   statIds:['dnf-mobile-dragon','luoke-s2','fengzhigu-refund','dnf-pc-harmony','moer-manor','hpmagic-pay'],
   cases:[['hpmagic-pay','哈利波特'],['moer-manor','摩尔庄园'],['fengzhigu-refund','枫之谷'],['luoke-s2','洛克王国'],['dnf-mobile-dragon','DNF手游'],['dnf-pc-harmony','DNF端游']]},
  {g:'情感服务类（乙游 / 二游）', color:'#B85450', contract:'"情感专属"', line:'OOC / 人设崩塌 / 涨价即背叛',
   statIds:['lianyu-mechanic','lianyu-scale','shijiezhiwai-money','lianyu-aoyin','chenbai-forbidden','bluearchive-cn','gf2-daiyan'],
   cases:[['lianyu-aoyin','恋与·敖尹'],['lianyu-scale','恋与·数值'],['lianyu-mechanic','恋与·机制'],['shijiezhiwai-money','世界之外'],['gf2-daiyan','少前2'],['bluearchive-cn','蔚蓝档案'],['chenbai-forbidden','尘白禁区']]},
  {g:'竞技 / 射击', color:'#3B6EA5', contract:'"规则公平"', line:'削弱 / 改契约即触雷',
   statIds:['oncehuman-season','diablo4-p11','apex-bp','sanjiaozhou-jail','codm-cn-launch','wzry-s40','wzry-world-demo','helldivers2-psn','helldivers2-nerf','chenbai-forbidden'],
   cases:[['helldivers2-psn','潜兵2·PSN'],['apex-bp','Apex'],['sanjiaozhou-jail','三角洲'],['helldivers2-nerf','潜兵2·削弱'],['wzry-s40','王者荣耀'],['codm-cn-launch','使命召唤'],['oncehuman-season','七日世界'],['diablo4-p11','暗黑4'],['wzry-world-demo','王者世界'],['chenbai-forbidden','尘白禁区']]},
  {g:'抽卡 RPG', color:'#8E7CC3', contract:'"资产保值"', line:'实装后改 / 暗改数值零容忍',
   statIds:['yuanshen-longwang','yuanshen-1year','bluearchive-cn','reverse1999-3rd'],
   cases:[['reverse1999-3rd','重返1999'],['yuanshen-longwang','原神·那维'],['yuanshen-1year','原神周年'],['bluearchive-cn','蔚蓝档案']]},
];
function renderC2Redline(){
  ...
  C2_REDLINE.forEach(r=>{
    ...
    (r.statIds||[]).forEach(id=>{
      ...
      if(t==='high')hi++; else if(t==='mid')mid++; else if(t==='low')lo++;
    });
    const eff=hi+mid+lo;
    rate[r.g]={mh:eff?Math.round((hi+mid)/eff*100):0, n:eff};
  });
  wrap.innerHTML = `<div class="c2RedGrid">${
    C2_REDLINE.map(r=>{
      // 高伤害(S档)案例排到最前，其余保持原顺序（稳定排序）
      const isHi=([id])=>{const c=byId[id];return c&&dmgTier3(c.damage)==='high';};
      const ordered=r.cases.map((x,i)=>[x,i])
        .sort((a,b)=>(isHi(b[0])-isHi(a[0]))||(a[1]-b[1]))
        .map(x=>x[0]);
      const chips=ordered.map(([id,nm])=>{
        const hi = isHi([id]);
        // 高伤害案例：用本品类颜色描边并高亮
        const cls = hi ? 'conclCase c2RedHi' : 'conclCase';
        const st  = hi ? ` style="border-color:${r.color};color:${r.color}"` : '';
      ...
      return `<div class="c2RedItem" style="border-left-color:${r.color}">
        ...
        <div class="d">契约是${r.contract}，<b>${r.line}</b>。</div>
        <div class="cs">${chips}</div>
        <div class="c2RedRate">
          <div class="lbl">踩线案例中高伤害率（n=${rt.n}）</div>
          <div class="row"><div class="track"><div class="fill" style="width:${rt.mh}%;background:${r.color}"></div></div><span class="val" style="color:${r.color}">${rt.mh}%</span></div>
        </div>
      </div>`;
    }).join('')
  }</div>`;
...
function renderC2PrChart(){
  ...
  const agg={}; order.forEach(p=>agg[p]={high:0,mid:0,low:0,na:0});
  (caseSummaries||[]).forEach(c=>{ if(agg[c.pr]) agg[c.pr][dmgTier3(c.damage)]++; });
  wrap.innerHTML = order.map(p=>{
    ...
    return `<div style="display:flex;align-items:center;gap:10px;margin:8px 0">
      ...
      <div style="flex:1 1 auto;background:var(--soft);border-radius:5px;height:22px;overflow:hidden">
        <div style="height:100%;width:${mh}%;background:${col};border-radius:5px"></div>
      </div>
...
// —— C2 图7a · 性别议题案例分布（两桶合并柱：2020-2023 / 2024-2026）——
function renderC2GenderTrend(){
  const wrap=document.getElementById('c2GenderTrendChart');
  if(!wrap) return;
  // 含「圈层矛盾-性别」的案例，按 ≤2023 / ≥2024 分两桶，合并总数
  const buckets=[{label:'2020–2023',n:0},{label:'2024–2026',n:0}];
  (caseSummaries||[]).forEach(c=>{
    if(!(c.tags||[]).some(t=>t==='圈层矛盾-性别')) return;
    const y=parseInt((c.time||'').slice(0,4),10); if(!y) return;
    (y<=2023?buckets[0]:buckets[1]).n++;
  });
  const W=440,H=250,padL=34,padR=18,padT=34,padB=44;
  const plotW=W-padL-padR, plotH=H-padT-padB;
  const maxV=Math.max(1,...buckets.map(b=>b.n));
  const slot=plotW/buckets.length, bw=Math.min(96,slot*0.46);
  const yy=v=> padT+plotH-(v/maxV)*plotH;
  let grid='',bars='',axis='';
  for(let v=0;v<=maxV;v++){grid+=`<line x1="${padL}" y1="${yy(v).toFixed(1)}" x2="${W-padR}" y2="${yy(v).toFixed(1)}" stroke="var(--line)" stroke-dasharray="${v?'2,4':'0'}"/><text x="${padL-6}" y="${(yy(v)+3).toFixed(1)}" text-anchor="end" font-size="10" fill="var(--muted)">${v}</text>`;}
  buckets.forEach((b,i)=>{
    const cx=padL+slot*i+slot/2, bx=cx-bw/2;
    const h=(b.n/maxV)*plotH, yTop=padT+plotH-h;
    const col=i===1?'#B85450':'#C08A8A';
    if(b.n){
      bars+=`<rect x="${bx.toFixed(1)}" y="${yTop.toFixed(1)}" width="${bw}" height="${h.toFixed(1)}" rx="4" fill="${col}"><title>${b.label} 性别议题案例 ${b.n} 例</title></rect>`;
      bars+=`<text x="${cx.toFixed(1)}" y="${(yTop-9).toFixed(1)}" text-anchor="middle" font-size="20" font-weight="800" fill="var(--text)">${b.n}</text>`;
    }
    axis+=`<text x="${cx.toFixed(1)}" y="${(padT+plotH+22).toFixed(1)}" text-anchor="middle" font-size="13" font-weight="600" fill="var(--text)">${b.label}</text>`;
  });
  wrap.innerHTML=`<svg viewBox="0 0 ${W} ${H}" width="100%" style="max-width:460px;display:block;margin:0 auto"><line x1="${padL}" y1="${padT+plotH}" x2="${W-padR}" y2="${padT+plotH}" stroke="var(--line)"/>${grid}${bars}${axis}</svg>`;
}

// —— C2 图7b · "数值/付费"类 vs "掌控/解释"类 年度折线 ——
function renderC2CtrlTrend(){
  const wrap=document.getElementById('c2CtrlTrendChart');
  if(!wrap) return;
  const VALUE=new Set(['数值/平衡争议','付费内容贬值/不实']);
  // 掌控/解释类：情感价值观争议、自创玩法管控、圈层内部撕裂、产能排期
  const CTRL=new Set(['情感/价值观争议','自创玩法管控争议','圈层矛盾-内部','产能/优先级排期']);
  const ym={};
  (caseSummaries||[]).forEach(c=>{
    const y=(c.time||'').slice(0,4); if(!y) return;
    ym[y]=ym[y]||{value:0,ctrl:0};
    const tags=new Set(c.tags||[]);
    if([...tags].some(t=>VALUE.has(t))) ym[y].value++;
    if([...tags].some(t=>CTRL.has(t))) ym[y].ctrl++;
  });
  const years=Object.keys(ym).sort();
  const series=[['value','数值 / 付费类','#3B6EA5'],['ctrl','掌控 / 解释类','#B85450']];
  const W=440,H=230,padL=30,padR=14,padT=18,padB=42;
  const plotW=W-padL-padR, plotH=H-padT-padB;
  const maxV=Math.max(1,...years.flatMap(y=>series.map(([k])=>ym[y][k]||0)));
  const x=i=> padL + (years.length>1? plotW*i/(years.length-1):plotW/2);
  const yy=v=> padT+plotH-(v/maxV)*plotH;
  let grid='',axis='',lines='';
  for(let v=0;v<=maxV;v++){grid+=`<line x1="${padL}" y1="${yy(v).toFixed(1)}" x2="${W-padR}" y2="${yy(v).toFixed(1)}" stroke="var(--line)" stroke-dasharray="${v?'2,4':'0'}"/><text x="${padL-6}" y="${(yy(v)+3).toFixed(1)}" text-anchor="end" font-size="10" fill="var(--muted)">${v}</text>`;}
  years.forEach((y,i)=>{axis+=`<text x="${x(i).toFixed(1)}" y="${(padT+plotH+18).toFixed(1)}" text-anchor="middle" font-size="11" fill="var(--muted)">${y}</text>`;});
  series.forEach(([k,,col])=>{
    let d=''; const pts=[];
    years.forEach((y,i)=>{const v=ym[y][k]||0; const px=x(i),py=yy(v); d+=(i?'L':'M')+px.toFixed(1)+' '+py.toFixed(1)+' '; pts.push([px,py]);});
    lines+=`<path d="${d}" fill="none" stroke="${col}" stroke-width="2.5"/>`;
    pts.forEach(([px,py])=>{lines+=`<circle cx="${px.toFixed(1)}" cy="${py.toFixed(1)}" r="4" fill="#fff" stroke="${col}" stroke-width="2.5"/>`;});
  });
  const svg=`<svg viewBox="0 0 ${W} ${H}" width="100%" style="max-width:460px;display:block;margin:0 auto">${grid}${lines}${axis}</svg>`;
  const legend=`<div class="gdLegend" style="justify-content:center;margin-top:2px">${
    series.map(([,nm,col])=>`<span class="gdLegendItem"><span style="display:inline-block;width:14px;height:2.5px;background:${col};vertical-align:middle;margin-right:4px"></span>${nm}</span>`).join('')
  }</div>`;
  wrap.innerHTML=svg+legend;
}

// —— C2 图2a · 生命周期阶段 × 高伤害率（竖向堆叠柱：去掉回炉/停运，突出 58%）——
function renderC2Lifecycle(){
  ...
  const sub={'上线期':'首年内','成长期':'1–3年','成熟期':'3年+'};
  const agg={}; order.forEach(s=>agg[s]={high:0,mid:0,low:0,na:0});
  // 射雕(回炉/停运,S级)在本图并入上线期口径：与右侧上线期根因图的 8 案保持一致
  (caseSummaries||[]).forEach(c=>{ let s=lcStage(c); if(c.id==='shediao-bjd') s='上线期'; if(!agg[s])return; agg[s][dmgTier3(c.damage)]++; });
  const C=DMG3_COLORS;
  ...
  rows.forEach((r,i)=>{
    ...
    [['low',C.low,'#fff'],['mid',C.mid,'#fff'],['high',C.high,'#fff']].forEach(([k,col,tc])=>{
      ...
      bars+=`<rect x="${bx.toFixed(1)}" y="${yTop.toFixed(1)}" width="${bw}" height="${h.toFixed(1)}" fill="${col}"><title>${label[r.s]} · ${DMG3_LABELS[k]}伤害 ${v}</title></rect>`;
...
function renderConclusions2(){
  renderC2TagDamage();
  renderC2YoConcentration();
  renderC2Lifecycle(); // 专用生命周期柱（去回炉/停运，突出 58%）
  renderC2LaunchRoots();
  renderC2Breakout();
  renderC2ContraBreakout();
  renderC2Redline();
  renderC2PrChart();
  renderC2GenderTrend();
  renderC2CtrlTrend();
