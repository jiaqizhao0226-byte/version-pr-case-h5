...
function renderCauseV1(c){
  ...
  const cognitionBlock=(ca.cognitionLead||traits)?`<div class="block ctBlock"><h3>玩家认知特征</h3>${ca.cognitionLead?`<p class="ctLead">${esc(ca.cognitionLead)}</p>`:''}<p class="muted">下列特征区分“游戏玩家普遍共性”与“本类游戏 / 本案玩家特定矛盾”，后续将汇总多案例横向归纳。</p>${traits?`<div class="ctGrid">${traits}</div>`:''}</div>`:'';

  return conflictBlock+journeyBlock+cognitionBlock;
}
...
/* 影响量化：① 结论(直接作标题) → ② 影响量化评估(徽章 + 三卡 + 可切换面板) → ③ 其他影响 */
function renderImpactV1(c){
  ...
  // ② 影响量化评估
  // — 面板：matrix（默认）/ vol / player / data
  const metrics=(q.volumeMetrics||[]).map(m=>`<article><b>${esc(m.value)}</b><span>${esc(m.label)}</span><p>${esc(m.note)}</p></article>`).join('');
  const volPane=`${verdict('声量',q.volumeVerdict)}<div class="volHead"><span class="volLevel">声量 ${esc(q.volumeLevel)} 级</span><span class="volConf">置信度：${esc(q.volumeConfidence)}</span></div><div class="impactMetricGrid">${metrics}</div>`;

  const ea=q.emotionAction||{};const au=q.audience||{};
  const auWhy=au.why?`<p class="audWhy"><b>为什么愤怒：</b>${esc(au.why)}</p>`:'';
  ...
  const playerPane=`${verdict('玩家是谁',au.verdict)}${auWhy}${eaInner?`<h4>情绪与行动</h4>${eaInner}`:''}`;

  const charts=(ds.charts||[]).map(ch=>renderBarTrend(ch,ds)).join('');
  const dmgRow=dmg.grade?`<div class="dmgRow">${ratePill('伤害评级',dmg.grade,dmg.tone)}${dmg.note?`<span class="dmgNote">${esc(dmg.note)}</span>`:''}</div>`:'';
  const dataPane=charts?`${verdict('数据结论',ds.verdict)}${dmgRow}${ds.why?`<p class="dataWhy">${esc(ds.why)}</p>`:''}<div class="dataCharts">${charts}</div>${ds.caption?`<p class="muted dataCaption">${esc(ds.caption)}</p>`:''}`:'<p class="muted">暂无脱敏数据。</p>';

  const cellFn=(key,name,desc)=>{const on=quad.resultCell===key;return `<div class="qmCell${on?' qmActive qm-'+key:''}"><b>${name}</b>${desc?`<small>${desc}</small>`:''}${on?'<span class="qmHere">← 本案落点</span>':''}</div>`;};
  const matrix=`<div class="quadMatrix"><div class="qmCorner">声量 ↓ ／ 数据 →</div><div class="qmColHead">数据动了</div><div class="qmColHead">数据没动</div><div class="qmRowHead">声量来自<br>承重墙</div>${cellFn('crisis','真危机','')}${cellFn('deficit','信任赤字累积','延迟爆发')}<div class="qmRowHead">声量来自<br>非承重墙</div>${cellFn('silent','沉默流失','最易误判')}${cellFn('noise','真噪声','')}</div>`;
  const matrixPane=matrix;

  // — 三张卡（带色码结论标签）
  const cards=[['vol','声量','akVol','大不大',quad.axis1],['player','玩家','akWall','是否承重墙',quad.axis2],['data','数据','akData','动没动',quad.axis3]];
  ...
  // 评级药丸统一放在卡片左下角：声量卡=声量等级，数据卡=伤害等级
  const cardRate=k=>{if(k==='vol'&&q.volumeLevel)return `<div class="cardFoot">${ratePill('声量',q.volumeLevel,volTone)}</div>`;if(k==='data'&&dmg.grade)return `<div class="cardFoot">${ratePill('伤害',dmg.grade,dmg.tone)}</div>`;return '';};
  const assessCards=`<div class="axisRow assessCards">${cards.map(a=>`<div class="axisCard assessCard${toneCls(a[0])}" data-imp="${a[0]}" role="button" tabindex="0" onclick="showImpPanel('${a[0]}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();showImpPanel('${a[0]}')}"><div class="axisHead"><span class="axisKey ${a[2]}">${a[1]}</span><span class="axisSub">${a[3]}</span>${cardTag(a[0])}</div><p>${strip(a[4])}</p>${cardRate(a[0])}</div>`).join('')}</div>`;

  const cellName=(({crisis:'真危机',deficit:'信任赤字累积',silent:'沉默流失',noise:'真噪声'}[quad.resultCell])||'待判定')+(quad.doubt?'（存疑）':'');
  const assessBlock=`<div class="block assessBlock"><div class="assessHead"><h3>影响量化评估</h3><button class="landBadge lb-${esc(quad.resultCell||'')} active" onclick="showImpPanel('matrix')">落点 · ${esc(cellName)}</button></div>${assessCards}<div class="assessPanel"><div class="impPane" data-pane="matrix">${matrixPane}</div><div class="impPane" data-pane="vol" hidden>${volPane}</div><div class="impPane" data-pane="player" hidden>${playerPane}</div><div class="impPane" data-pane="data" hidden>${dataPane}</div></div></div>`;

  // ③ 其他影响
  const pi=(q.productImpact||[]).filter(x=>!/商业数据/.test(x.name||''));
...
/* 影响量化评估：点卡片 → 面板原地切换到该维度详情；点徽章 → 回四象限矩阵 */
function showImpPanel(which){
  ...
  const badge=document.querySelector('.landBadge'); if(badge) badge.classList.toggle('active',which==='matrix');
}
...
/* 案例启发（迁移 V1）：整合为单一「关键启发」列表——只留最有代表性、可复用的点，不分模块 */
function renderInsightV1(c){
  ...
  if(ki) return `<div class="block ccBlock"><h3>关键启发</h3><div class="ccGrid">${ki}</div></div>`;
  // 兜底：旧字段（无 keyInsights 时）
