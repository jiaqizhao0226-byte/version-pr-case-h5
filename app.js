...
function renderJourneyStages(c){
  ...
  return `<div class="journeyTrend"><div class="trendHeader"><div><h4>玩家情绪强度趋势图</h4><p class="muted">横轴为事件阶段，纵轴为玩家情绪强度。折线越高，代表该阶段玩家愤怒、质疑和行动化倾向越强。</p></div></div>${renderEmotionLineChart(stages)}<div class="stageStoryList">${stages.map((s,i)=>`<article class="stageStory"><header><span>阶段${i+1}</span><div><h4>${s.stage.replace(/^阶段[一二三四五六七八九十]+：/,'')}</h4><p>${s.time}｜${s.emotion}</p></div></header><section class="storyMain"><div class="storyPoint">${s.stageSummary||s.coreQuestion}</div><p><b>玩家怎么想：</b>${s.psychology}</p><p><b>玩家要什么：</b>${s.playerDemand||''}</p><p><b>触发因素：</b>${s.trigger||''}</p></section><section class="storyEvidence"><h5>玩家反馈截图 / 摘录</h5>${(s.evidence||[]).map(e=>`<a class="commentShot" target="_blank" href="${e.url}"><div class="commentShotTop"><span class="avatar">${(e.platform||'评').slice(0,1)}</span><div><b>${e.platform||'公开平台'} · ${e.sourceType||'玩家评论'}</b><small>${[e.time,e.sourceTitle].filter(Boolean).join('｜')}</small></div></div><div class="commentShotText">${e.text}</div><div class="commentShotFoot"><span>${e.heat||'热度待补'}</span><span>点击查看来源</span></div></a>`).join('')}</section></article>`).join('')}</div></div>`;
}
