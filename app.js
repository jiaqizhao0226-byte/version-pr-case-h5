...
function renderJourneyStages(c){
  ...
  return `<div class="journeyTrend"><div class="trendHeader"><div><h4>玩家情绪强度趋势图</h4><p class="muted">横轴为事件阶段，纵轴为玩家情绪强度。折线越高，代表该阶段玩家愤怒、质疑和行动化倾向越强。</p></div></div>${renderEmotionLineChart(stages)}<div class="stageCards simplified">${stages.map((s,i)=>`<article class="journeyStage simplified"><div class="stageIndex">${i+1}</div><div class="stageBody"><div class="stageTop"><div><b>${s.stage}</b><span>${s.time}</span></div><em>${s.emotion}</em></div><div class="stageFocus"><span>本阶段重点</span><p>${s.stageSummary||s.coreQuestion}</p></div><div class="stageTwoCol"><section><h5>玩家怎么想</h5><p>${s.psychology}</p></section><section><h5>玩家要什么</h5><p>${s.playerDemand||''}</p></section></div><div class="stageTrigger"><b>触发因素：</b>${s.trigger||''}</div><div class="stageEvidence compact"><h5>对应证据</h5>${(s.evidence||[]).map(e=>`<a target="_blank" href="${e.url}"><div class="evidenceMeta"><span>${e.type}</span>${e.platform?`<b>${e.platform}</b>`:''}${e.time?`<b>${e.time}</b>`:''}${e.sourceType?`<b>${e.sourceType}</b>`:''}</div><strong>${e.text}</strong>${e.sourceTitle?`<small>出处：${e.sourceTitle}</small>`:''}${e.heat?`<small>热度：${e.heat}</small>`:''}</a>`).join('')}</div></div></article>`).join('')}</div></div>`;
}
