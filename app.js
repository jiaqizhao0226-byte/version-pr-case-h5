...
function renderJourneyStages(c){
  ...
  return `<div class="journeyTrend"><div class="trendHeader"><div><h4>玩家情绪强度趋势图</h4><p class="muted">横轴为事件阶段，纵轴为玩家情绪强度。折线越高，代表该阶段玩家愤怒、质疑和行动化倾向越强。</p></div></div>${renderEmotionLineChart(stages)}<div class="stageCards clean">${stages.map((s,i)=>`<article class="journeyStage clean"><div class="stageIndex">${i+1}</div><div class="stageBody"><div class="stageTop"><div><b>${s.stage}</b><span>${s.time}</span></div><em>${s.emotion}</em></div><p class="stageLead">${s.stageSummary||s.coreQuestion}</p><div class="stageNarrative"><p><b>玩家怎么想：</b>${s.psychology}</p><p><b>玩家要什么：</b>${s.playerDemand||''}</p><p><b>触发因素：</b>${s.trigger||''}</p></div><div class="stageEvidence inline"><h5>对应证据</h5>${(s.evidence||[]).map(e=>`<a target="_blank" href="${e.url}"><strong>${e.text}</strong><small>${[e.platform,e.time,e.sourceType,e.sourceTitle,e.heat?`热度：${e.heat}`:''].filter(Boolean).join('｜')}</small></a>`).join('')}</div></div></article>`).join('')}</div></div>`;
}
