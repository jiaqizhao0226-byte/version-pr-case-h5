...
function renderDetail(c){
  ...
  const tabs=[['timeline','时间线'],['players','玩家爆发'],['official','官方处置'],['data','影响量化'],['insight','案例启发']];
...
function tab(id){
  ...
  const names={timeline:'时间线',players:'玩家爆发',official:'官方处置',data:'影响量化',insight:'案例启发'};
  [...document.querySelectorAll('.tab')].find(x=>x.textContent===names[id]).classList.add('active');
  $(`tab-${id}`).classList.add('active');
}

function renderTab(c,id){
  if(id==='timeline')return `<div class="block"><h3>T-window 时间线</h3><div class="timeline">${c.timeline.map(e=>`<div class="event ${e.side}"><div class="time"><span class="side">${e.side==='official'?'官方动作':'玩家动作'}</span>${e.phase} / ${e.time}</div><div class="name">${e.event}</div><div class="impact">${e.impact}</div></div>`).join('')}</div></div><div class="block"><h3>T-window记录规则</h3><p class="muted">所有官方单独动作都要拆开：前瞻、上线/热更新、首次回应、修复/回滚、高层公开信、补偿加码、后续机制承诺均单列。玩家侧反应、扒改动、质疑、翻旧账、是否原谅等也要单列到玩家动作侧，不能写在官方动作卡片里。</p></div>`;
  ...
  if(id==='data')return `<div class="grid2"><div><div class="block"><h3>B站高赞评论</h3><table class="table"><tr><th>评论</th><th>点赞</th><th>反映心态</th></tr>${c.quotes.map(q=>`<tr><td>${q[0]}</td><td>${q[1]}</td><td>${q[2]}</td></tr>`).join('')}</table></div></div><aside><div class="block"><h3>声量-伤害矩阵</h3><div class="matrix"><div><b>声量</b><p>${c.volume}</p></div><div><b>伤害</b><p>${c.damage}</p></div><div><b>状态</b><p>${c.status}</p></div><div><b>样本</b><p>674条热门评论</p></div></div></div><div class="block source"><h3>来源</h3>${c.sources.map(x=>`<a target="_blank" href="${x}">${x}</a>`).join('')}</div></aside></div>`;
  if(id==='insight')return `<div class="block"><h3>案例启发：玩家认知变化与未来治理</h3><p class="muted">这一页把“玩家如何重新理解事件”和“未来应该如何预防/处理”放在一起看，避免把洞察和行动拆散。</p><h4>玩家认知变化</h4>${c.cognition.map(x=>`<div class="quote">${x}</div>`).join('')}<h4>被破坏的默认契约</h4><div class="chips">${c.tags.map(t=>`<span class="chip">${t}</span>`).join('')}</div><h4>未来启发与治理清单</h4><ul>${c.lessons.map(x=>`<li>${x}</li>`).join('')}</ul><div class="quote">核心判断：本案的启发不是“动作不要改”，而是任何会影响已上线内容、情感资产和玩家解释权的调整，都必须前置公告、说明决策链，并优先准备共存方案。</div></div>`;
}
