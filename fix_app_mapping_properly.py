...
# Make mapping table render inside mapping tab
# Also ensure openCase is intact
render_grid = """
function renderGrid(){
  renderStats();
  const list=filtered();
...
js = re.sub(r'function renderGrid\(\)[\s\S]*?function goHome\(\)\{location\.hash=\'\'\;\}', render_grid.strip(), js)
