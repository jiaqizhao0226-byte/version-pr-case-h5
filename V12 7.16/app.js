...
const C2_REDLINE = [
  ...
  {g:'情感陪伴类（乙游 / 二游）', color:'#B85450', contract:'"情感专属"', line:'陪伴断供或专属被背叛即崩塌',
   statIds:['lianyu-aoyin','gf2-daiyan','benghuai-rabbit'],
   criteria:[
     {t:'现有角色停更断供，玩家的情感陪伴需求得不到持续满足', cases:[['lianyu-aoyin','恋与·敖尹']]},
     {t:'角色 OOC、差别化演出或情感移情，背叛专属关系', cases:[['gf2-daiyan','少前2·黛烟'],['benghuai-rabbit','崩坏3·兔女郎']]},
   ]},
...
function renderC2Redline(){
  ...
  C2_REDLINE.forEach(r=>{
    ...
    const eff=hi+mid+lo;
    rate[r.g]={mh:eff?Math.round((hi+mid)/eff*100):0, hit:hi+mid, n:eff, items};
  });
  const isMidHigh=id=>{const c=byId[id];const tier=c&&dmgTier3(c.damage);return tier==='high'||tier==='mid';};
  const chipHtml=(id,nm,color)=>{
    const midHigh=isMidHigh(id);
    const cls=midHigh?'conclCase c2RedHi':'conclCase';
    const st =midHigh?` style="border-color:${color};color:${color}"`:'';
    ...
    C2_REDLINE.map(r=>{
      ...
      if(r.criteria){
        ...
        body = `<ol class="c2RedCrit">${
          r.criteria.map(cr=>{
            const ordered=cr.cases.slice().sort((a,b)=>isMidHigh(b[0])-isMidHigh(a[0]));
        ...
        const ordered=r.cases.map((x,i)=>[x,i])
          .sort((a,b)=>(isMidHigh(b[0][0])-isMidHigh(a[0][0]))||(a[1]-b[1]))
          .map(x=>x[0]);
        const chips=ordered.map(([id,nm])=>chipHtml(id,nm,r.color)).join('');
      ...
      return `<div class="c2RedItem" style="border-left-color:${r.color}">
        ...
        ${body}
        <div class="c2RedRate">
          <div class="lbl">踩线案例中高伤害数 / 总样本数</div>
          <div class="row" title="${esc(`${r.g}踩线案例：${c2CaseList(rt.items)}`)}"><div class="track"><div class="fill" style="width:${rt.mh}%;background:${r.color};cursor:help"></div></div><span class="val" style="color:${r.color}">${rt.hit}/${rt.n}</span></div>
        </div>
      </div>`;
    }).join('')
  }</div>`;
...
const C4_GROUPS=[
  {
    ...
    items:[
      {
        match:'玩法生态', tag:'用户趋势',
        title:'玩家从玩法消费者走向玩法生态的共同生产者',
        thesis:'玩法供给权开始由官方单向生产，转向官方与玩家共同生产：一类由社群在既有机制中自发形成，另一类由厂商开放工具、发布、分发与激励体系规模化生产。',
      ...
      {
        match:'饭圈化组织技术', tag:'用户圈层',
        title:'高情感投入叠加饭圈化组织技术，使乙游圈层更容易形成高声势与现实压力',
        thesis:'亲密关系型产品结构放大玩家情绪，成熟社群手段再把情绪转化为统一叙事、集体行动与外部施压。',
  ...
  },
  {
    ...
    items:[
      {
        match:'生态位提供风险缓冲', tag:'生态位',
        title:'强生态位提供风险缓冲，也影响官方处置姿态',
        thesis:'',
        template:'c4-niche-template'
      },
      {
        match:'识别「真实反馈」与「噪声」', tag:'官方决策',
        title:'识别「真实反馈」与「噪声」：判断哪些问题值得进入版本计划',
        thesis:'',
        template:'c4-priority-template'
      },
      {
        match:'常态版本机制', tag:'修复闭环',
        title:'将问题修复纳入常态版本机制，持续兑现比单次回应更能恢复信任',
        thesis:'',
        template:'c4-repair-loop-template'
      },
      {
        match:'回应质量', tag:'官方回应',
  ...
  C4_GROUPS.forEach(group=>{
    ...
    group.items.forEach(item=>{
      ...
      if(item.thesis){
        ...
        thesis.className='c4ThesisInside';
        thesis.textContent=item.thesis;
        expanded.appendChild(thesis);
      }
      if(sourceTemplate){
      ...
      if(item.enhance==='pressureStructure'){
        ...
        const model=expanded.querySelector('.c2PressureModel');
        ...
        if(body&&model) model.setAttribute('aria-label','高情感投入与饭圈化组织技术共同放大乙游圈层行动力');
        if(body&&model){
          ...
          evidence.innerHTML=`
            <div class="c2PressureEvidenceHead">
              <span>案例验证</span>
              <h3>女性向 / 情感乙游特有的亲密关系投入，经组织化行动后更容易转化为现实压力</h3>
            </div>
            <div class="c2PressureEvidenceGrid">
              <article>
                <div class="c2PressureEvidenceTag">主要体现 · 组织化执行</div>
                <a onclick="openCase('lianyu-mechanic')">恋与深空 · 暗蚀国王</a>
                <p>核心付费内容与期待不符触发受损感，玩家将分散意见整理为统一诉求，并以联名、停氪联盟、集中投诉与律师函持续施压。</p>
                <div class="c2PressureEvidencePath"><b>付费契约受损</b><i>→</i><b>统一诉求</b><i>→</i><b>集中行动</b><i>→</i><b>律师函</b></div>
                <small><b>案例落点：</b>成熟组织手段使个人不满迅速转化为可持续的集体行动。</small>
              </article>
              <article class="isKey">
                <div class="c2PressureEvidenceTag">主要体现 · 情绪与行动叠加</div>
                <a onclick="openCase('lianyu-aoyin')">恋与深空 · 敖尹</a>
                <p>长期内容债与新增男主共同触发关系背叛感，玩家通过停氪、停登、投诉及公共议题传播持续抬高处置压力。</p>
                <div class="c2PressureEvidencePath"><b>关系背叛感</b><i>→</i><b>群体共振</b><i>→</i><b>组织化行动</b><i>→</i><b>战略回撤</b></div>
                <small><b>案例落点：</b>高情绪强度与成熟行动手段叠加，最终推动不可逆的产品策略调整。</small>
              </article>
              <article>
                <div class="c2PressureEvidenceTag">主要体现 · 外部施压</div>
                <a onclick="openCase('shijiezhiwai-money')">世界之外 · 召唤之王</a>
                <p>未预告涨价同时损害消费信任，玩家通过固定证据、停氪罢玩及消保渠道，将付费不满转化为消费者权益问题。</p>
                <div class="c2PressureEvidencePath"><b>消费信任受损</b><i>→</i><b>固定证据</b><i>→</i><b>消保 / 12315</b><i>→</i><b>权益压力</b></div>
                <small><b>案例落点：</b>组织化行动将圈内争议接入外部机构能够识别和处理的风险接口。</small>
              </article>
            </div>
          `;
          model.insertAdjacentElement('afterend',evidence);

          if(conclusion){
            const conclusionText=conclusion.querySelector('p');
            if(conclusionText) conclusionText.innerHTML='<b>乙游的高强度亲密关系放大情绪，饭圈化组织技术放大行动。</b>两者叠加，使圈层诉求更容易形成持续声量，并推动企业、平台或外部机构作出现实响应。';
      ...
      if(item.enhance==='genderDataConclusion'){
        ...
        dataSub?.remove();
        const typeTitle=sectionHeads[1]?.querySelector('b');
        const typeSub=sectionHeads[1]?.querySelector('small');
...
async function route(){
  ...
  if(id){
    ...
    if(tabId==='mapping'||tabId==='conclusions') tabId='overview';
    switchMainTab(tabId);
    renderGrid();
  }
}
